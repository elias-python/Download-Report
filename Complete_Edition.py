...
RECEPCAO_ASSETS = {
    ...
    "CATALÃO": "relatorio_recepcao_1.png",
    "SORRISO": "relatorio_recepcao_1.png",
    "PALMEIRANTE": "relatorio_recepcao_1.png",
...
class App(ctk.CTk):
    ...
    def salvar_config(self, caminho_desc=None, caminho_carr=None, caminho_rec=None, ultima_att=None, user=None, senha=None, csap1=None, csap2=None, csap3=None):
        ...
        if caminho_rec is not None: dados["caminho_recepcao"] = caminho_rec
    ...
    def selecionar_caminho_base(self):
        ...
        if "Ambos" in modo_atual:
            messagebox.showinfo("Atenção", "Selecione a aba 'Descarga', 'Carregamento' ou 'Recepção' individualmente para configurar.")
            return
    ...
    def _atualizar_dest(self):
        modo_atual = self.modo_var.get()
        if "Ambos" in modo_atual:
            if self.caminho_descarga and self.caminho_carregamento and self.caminho_recepcao:
                self.lbl_dest_path.configure(text="✔️ Todas as pastas configuradas.", text_color=COR_SUCCESS)
    ...
    def ao_mudar_modo(self, valor_selecionado):
        ...
        elif "Recepção" in valor_selecionado: self.tema_atual = TEMA_RECEPCAO
    ...
    def _build_ui(self):
        ...
        self.seg_button = ctk.CTkSegmentedButton(tab_atlas, values=["📥 Descarga", "📤 Carregamento", "📬 Recepção", "🔄 Ambos"], variable=self.modo_var, command=self.ao_mudar_modo, font=("Segoe UI", 13, "bold"), height=35)
    ...
    def verificar_senha_incorreta(self):
        ...
        if self.clicar_img("assets/aviso_senha_incorreta.png", "Aviso de Senha Incorreta", timeout=3, max_tentativas=1):
            self.adicionar_log("⚠️ AVISO: Senha incorreta! Clicando em OK e continuando...", "err")
            self.senha_incorreta = True
            # Clica no botão OK do aviso
            if self.clicar_img("assets/ok_senha_incorreta.png", "Botão OK", timeout=2, max_tentativas=1):
                time.sleep(1)
                return True
        return False
    ...
    def executar_robo_atlas(self, unidade, modo_especifico):
        ...
        try:
            ...
            if unidade in ["PGUA 1", "PGUA 2"]:
                ...
                if self.verificar_senha_incorreta(): return False
            ...
            if modo_especifico == "Descarga":
                ...
                if unidade in ["UBERABA", "RONDONÓPOLIS"]:
                    if self.clicar_img("assets/selectfluxo.png", "Fluxo"):
                        pyautogui.write("DESCARGA"); pyautogui.press('down'); pyautogui.press('enter')
                    else: return False
                else:
            ...
            # --- BLOCO DE RECEPÇÃO ---
                ...
                if not self.clicar_img(f"assets/{recepcao_asset}", "Relatório Recepção"): return False
                ...
                time.sleep(1.5) 
                if not self.clicar_img("assets/gerar_relatorio.png", "Gerar"): return False
                if not self.mover_arquivo_recepcao(unidade, hoje.strftime("%m.%Y")): return False
            ...
            # --- BLOCO DE CARREGAMENTO ---
                ...
                time.sleep(1.5) 
                if not self.clicar_img("assets/gerar_relatorio.png", "Gerar"): return False
                if not self.mover_arquivo_atlas(unidade, hoje.strftime("%m.%Y"), False): return False
    ...
    def mover_arquivo_recepcao(self, unidade, mes_ano):
        ...
        tempo_inicio = time.time()
        nums_recepcao = {
            "PGUA 1": "4020", "CANDEIAS": "4040", "SORRISO": "4060", 
            "RIO VERDE": "4070", "CATALÃO": "4100", "RONDONÓPOLIS": "4110", 
            "UBERABA": "4120", "PGUA 2": "4130", "RIO GRANDE": "4140", 
            "PALMEIRANTE": "4170"
        }
    ...
    def start_thread(self):
        ...
        if centros_atlas:
            if not self.ent_user.get().strip() or not self.ent_pass.get().strip():
                ...
                return
            modo_atual = self.modo_var.get()
            if "Ambos" in modo_atual and (not self.caminho_descarga or not self.caminho_carregamento or not self.caminho_recepcao):
                messagebox.showwarning("Atenção", "Configure as pastas de Descarga, Carregamento e Recepção.")
                return
    ...
    def executar_sequencial(self):
        ...
        if "Ambos" in modo_geral: modos_a_rodar = ["Descarga", "Carregamento", "Recepção"]
        elif "Descarga" in modo_geral: modos_a_rodar = ["Descarga"]
        elif "Recepção" in modo_geral: modos_a_rodar = ["Recepção"]
            ...
            for centro in centros_atlas:
                for modo in modos_a_rodar:
                    ...
                    if self.executar_robo_atlas(centro, modo):
                        ...
                        falhas += 1; lista_falhas.append(f"Atlas: {centro} ({modo})")
                    self.fechar_atlas()
                    time.sleep(1)
